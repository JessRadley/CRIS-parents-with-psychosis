...
#change group to numeric and remove the few from the file that have imd group as NA
file1_WS_child_inb$group<-as.numeric(file1_WS_child_inb$group)
file1_WS_child_inb<-file1_WS_child_inb%>%filter(!is.na(group))
...
#Survival analysis just with those with children (n= 1087 - 1094 with the ones with group missing but those have been removed)
...
#Backwards stepwise regression using AIC
step(mod1, direction = "backward", k = 2)
#Remove Employment then ethnicity then accommodation then no.of child then anyunder 18 then gender value

finalmod1<-coxph(Surv(WS_year, presence_of_WS) ~ CurrentAge + Marital_Status_Value + Diagnosis + Smoking + group, data = file2_WS_child_inb)
summary(finalmod1)
AIC(finalmod1) #7594.79
